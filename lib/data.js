...
/**
 * Fetch all cards with their price history.
 */
export async function fetchCardsWithPrices() {
  if (!supabase) throw new Error('Missing Supabase environment variables. Check NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY.')
  ...
  for (const row of history) {
    ...
    historyMap[row.card_id].push(row)
  }

  return cards.map(c => ({ ...c, priceHistory: historyMap[c.id] || [] }))
}

/**
 * Get unique month labels for card price history.
 */
export async function fetchCardMonthLabels() {
  if (!supabase) return []
